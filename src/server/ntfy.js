...
function notifySessionExit(sessionName, exitCode, ntfyConfig, meta) {
    const success = exitCode === 0;
    const parts = [];
    if (meta?.hostname)
        parts.push(`[${meta.hostname}]`);
    parts.push(`"${sessionName}"`);
    parts.push(success ? 'finished' : 'failed');
    if (meta?.durationMs !== undefined)
        parts.push(`in ${formatDuration(meta.durationMs)}`);
...
function notifySessionStopped(sessionName, reason, ntfyConfig) {
    notifySessionEvent({
...
function notifySessionEvent(event, ntfyConfig) {
