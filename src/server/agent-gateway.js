...
function createAgentGateway(httpServer, db, config, audit) {
    ...
    wss.on('connection', (ws, req) => {
        ...
        if (agentBroadcaster)
            agentBroadcaster('agent:status', { agentId, hostname: agent.hostname, status: 'online' });
                ...
                case 'hello': {
                    ...
                    try {
                        const shells = Array.isArray(msg.shells)
                            ...
                            : null;
                        db.prepare('UPDATE agents SET name=?, hostname=?, platform=?, shells=? WHERE id=?').run(msg.hostname || agent.hostname, msg.hostname || agent.hostname, msg.platform || agent.platform, shells, agentId);
                        reconcileAgentSessions(agentId, msg.sessions);
                        if (agentBroadcaster)
                            agentBroadcaster('sessions:changed', { agentId });
                    }
                    catch (_) { }
                    break;
                }
                case 'output': {
                    if (sessionAgentMap.get(msg.sessionId) !== agentId)
                        return;
        ...
        ws.on('close', () => {
            ...
            if (agentConnections.get(agentId) === ws) {
                ...
                if (agentBroadcaster)
                    agentBroadcaster('agent:status', { agentId, status: 'offline' });
    ...
    function registerSessionCallbacks(sessionId, agentId, { onOutput, onExit }) {
        ...
        if (!sessionBrowserCbs.has(sessionId))
            sessionBrowserCbs.set(sessionId, new Map());
    ...
    function reconcileAgentSessions(agentId, sessionIds) {
        ...
        const activeRows = db.prepare("SELECT id FROM sessions WHERE agent_id = ? AND status = 'active'").all(agentId);
    ...
    function appendPersistedOutput(sessionId, data) {
        try {
            db.prepare(`INSERT INTO session_output (session_id, data, updated_at)
         VALUES (?, ?, ?)
         ON CONFLICT(session_id)
         DO UPDATE SET
           data = substr(session_output.data || excluded.data, ?),
           updated_at = excluded.updated_at`).run(sessionId, data, Date.now(), -MAX_PERSISTED_OUTPUT_CHARS);
        }
        catch (_) { }
    }
