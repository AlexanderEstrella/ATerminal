...
function createBrowserWs(httpServer, db, config, agentGateway, audit) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('terminal:attach', (payload) => {
      ...
      if (previousCallbackId) agentGateway.removeSessionCallbacks(sessionId, previousCallbackId);
    ...
    socket.on('terminal:interrupt', ({ sessionId }: any = {}) => {
      const session = getSession(db, sessionId);
      if (!session || !canAccessSession(socket.user, session) || session.status !== 'active') return;
      const agentId = agentGateway.getAgentForSession(sessionId);
      if (agentId) {
    ...
    socket.on('terminal:resize', ({ sessionId, cols, rows }: any = {}) => {
      const session = getSession(db, sessionId);
      if (!session || !canAccessSession(socket.user, session)) return;
...
function createApiRouter(db, config, agentGateway, audit) {
  ...
  router.get('/sessions/:id/context', (req, res) => {
    ...
    const agent = db.prepare(
      'SELECT id, hostname, name, platform, last_seen FROM agents WHERE id = ?'
  ...
  router.post('/sessions', async (req, res) => {
    ...
    const agent = db.prepare(
      'SELECT id, hostname, name, platform, shells FROM agents WHERE id = ?'
    ).get(agentId);
    if (!agent) {
  ...
  router.get('/agents/:agentId/locations', async (req, res) => {
    ...
    const agent = db.prepare(
      'SELECT id, hostname, name, platform FROM agents WHERE id = ?'
    ).get(agentId);
    if (!agent) {
      return res.status(404).json({ error: 'Agent not found' });
    }
  ...
  router.delete('/agents/:id', (req, res) => {
    ...
    if (gw) gw.disconnectAgent(agentId);
  ...
  router.post('/locations/favorites', (req, res) => {
    ...
    if (pathError || !path) return res.status(400).json({ error: pathError || 'path is required' });
...
function serializeSessionSummaries(db: any, agentGateway: any, sessions: any[]) {
  const onlineAgents = new Set(agentGateway.listConnectedAgents());
  const agentsById = new Map((listAgents(db) as any[]).map((agent) => [agent.id, agent]));
...
function validatePathInput(value) {
  ...
  if (value.length > 2048) return 'path is too long';
  if (/[\0-\x08\x0B\x0C\x0E-\x1F]/.test(value)) return 'path contains invalid characters';
  return null;
}
