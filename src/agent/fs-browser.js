...
function resolveTargetPath(inputPath) {
  ...
  if (inputPath.length > MAX_PATH_LENGTH) throw new Error('Path is too long')
  return path.resolve(inputPath)
}

function parentOf(targetPath) {
  const parsed = path.parse(targetPath)
  if (targetPath === parsed.root) return null
...
function roots() {
  ...
  for (let code = 65; code <= 90; code++) {
    ...
    try {
      if (fsSync.existsSync(drive)) drives.push({ name: drive, path: drive })
    } catch {}
  }
