...
server
  .command('set-ntfy [url]')
  .description('Configure ntfy push notifications for session exits')
  .option('--token <token>', 'Access token for authenticated ntfy topics')
  .option('--disable', 'Remove ntfy configuration')
  .action((url, opts) => {
    const { readServerConfig, writeServerConfig } = require('../src/config');
    try {
      const config = readServerConfig();
      if (opts.disable) {
        delete config.ntfy;
      ...
      if (!url) {
        if (config.ntfy?.url) {
          console.log('ntfy URL:', config.ntfy.url);
          if (config.ntfy.token) console.log('ntfy token: (set)');
        } else {
          console.log('ntfy not configured.\nUsage: aterminal server set-ntfy <url> [--token <token>]');
        }
        return;
      }
      try { new URL(url); } catch { throw new Error('Invalid URL: ' + url); }
      config.ntfy = { url };
      if (opts.token) config.ntfy.token = opts.token;
      writeServerConfig(config);
      console.log('ntfy notifications configured.');
      console.log('  URL:', url);
...
async function syncAdminPasswordFromEnv(config) {
    ...
    if (!user)
        return;
    if (bcrypt.compareSync(adminPass, user.password_hash))
        return;
...
async function maybeAutoEnrollLocalAgent(config) {
    ...
    db.prepare(`INSERT INTO agents (id, name, hostname, platform, secret_hash, enrolled_at, status)
     VALUES (?, ?, ?, ?, ?, ?, 'offline')`).run(agentId, `${hostname} (local)`, hostname, process.platform, secretHash, Date.now());
    writeAgentConfig({ serverUrl: `http://127.0.0.1:${config.port}`, agentId, agentSecret });
    console.log(`\nThis machine enrolled as agent "${hostname} (local)".`);
    console.log('Starting local agent...');
    spawnAgent();
    function spawnAgent() {
        const child = spawn(process.execPath, ['--no-warnings=ExperimentalWarning', path.join(__dirname, 'aterminal.js'), 'agent', 'start'], { stdio: 'inherit', detached: false });
...
async function startTailscaleFunnel(config, opts) {
    ...
    if (!tailscaleBin) {
        throw new Error(`tailscale was not found. ${tailscaleInstallHint()}\n` +
    ...
    if (!dnsName) {
        throw new Error('Tailscale MagicDNS hostname not available.\n' +
    ...
    await new Promise((resolve, reject) => {
        ...
        child.on('error', (err) => {
            ...
            if (!settled) {
                settled = true;
                reject(new Error(`tailscale serve error: ${err.message}`));
            }
        });
        child.on('exit', (code) => {
            clearTimeout(timer);
...
function tailscaleInstallHint() {
    if (process.platform === 'win32')
        return 'Download from: https://tailscale.com/download/windows';
    if (process.platform === 'darwin')
        return 'Install with: brew install tailscale';
