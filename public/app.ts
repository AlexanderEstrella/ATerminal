...
const show = (el: any) => el.classList.remove('hidden');
const hide = (el: any) => el.classList.add('hidden');
...
async function renderQr(container: any, text: string) {
...
async function copyText(text: string, btn?: any) {
  try {
...
async function shareOrCopy(text: string, title: string, btn?: any) {
  if (navigator.share) {
    try {
      const payload: any = { title, text };
      if (text.startsWith('http')) payload.url = text;
...
$('#login-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    try {
      ...
      if (status && status.user) currentUser = status.user;
      if (status && status.publicUrl) serverPublicUrl = status.publicUrl;
    } catch (_) {}
    initDashboard();
  } catch (err) {
...
function renderAgentList(agents) {
  ...
  list.querySelectorAll('.unpair-btn').forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      if (!confirm(`Unpair ${btn.closest('.agent-card').querySelector('.agent-hostname').textContent}?`)) return;
      try {
...
function renderSessionList(sessions) {
  ...
  list.querySelector('.session-history-clear')?.addEventListener('click', async (e) => {
    ...
    if (!confirm('Delete all history sessions?')) return;
    try {
...
function buildSessionGroupsHtml(sessionList) {
  ...
  groups.forEach((groupSessions, agentId) => {
    ...
    groupSessions.forEach((s) => {
      ...
      if (s.cwd) sessionMeta.push(locationLabel(s.cwd));
      if (!isActive && s.ended_at) sessionMeta.push(`ended ${relativeTime(s.ended_at)}`);
...
function scheduleSessionContextRefresh(sessionId) {
  if (sessionContextRefreshTimer) clearTimeout(sessionContextRefreshTimer);
  sessionContextRefreshTimer = setTimeout(() => {
    if (activeSessionId === sessionId) loadSessionContext(sessionId);
  }, 800);
}
...
async function loadSessionContext(sessionId) {
  ...
  try {
    ...
    if (seq !== sessionContextSeq || activeSessionId !== sessionId) return;
...
function updateSessionContext(data, loading) {
  ...
  if (!titleEl || !metaEl || !outputEl || !activeSessionId) return;
...
function getDefaultShells(agent) {
  if (!agent) return ['powershell', 'cmd'];
  if (agent.platform === 'win32') return ['powershell', 'cmd', 'wsl'];
  if (agent.platform === 'darwin') return ['zsh', 'bash'];
  return ['bash', 'sh'];
}
...
async function loadSavedLocations(agentId, seq = locationAgentSeq) {
  ...
  if (seq !== locationAgentSeq) return;
...
async function browseLocation(path, seq = locationAgentSeq) {
  if (!locationState.agentId) return;
  ...
  try {
    ...
    if (seq !== locationAgentSeq || browseSeq !== locationBrowseSeq) return;
...
function renderLocationPicker() {
  ...
  if (cwdInput) cwdInput.disabled = !locationState.agentId;
...
async function toggleFavorite(path) {
  ...
  if (!locationState.agentId || !normalizedPath) return;
...
function normalizeDirectoryResponse(data, requestedPath) {
  ...
  const currentPath = normalizePath(
    source.path || source.cwd || source.currentPath || source.current || requestedPath || ''
  );
...
function normalizeDirectoryItem(item, currentPath) {
  ...
  if (!path && item.name) path = joinDisplayPath(currentPath, item.name);
  if (!path) return null;
...
function setSessionCwd(path) {
  const cwdInput = $('#session-cwd');
  if (cwdInput) cwdInput.value = normalizePath(path);
}
...
function locationLabel(path) {
  ...
  if (!normalized) return 'Agent default';
...
function buildPreviewUrl(port) {
  if (!serverPublicUrl) return `http://localhost:${port}`;
  try {
...
function activateTab(sessionId: string) {
  ...
  document.querySelectorAll('.session-item').forEach((t: any) =>
    t.classList.toggle('active', t.dataset.id === sessionId));
...
async function handleDeviceApproval(id: string, action: string) {
  try {
...
function buildEnrollmentCommand(serverUrl: string, enrollmentToken?: string) {
  const safeServerUrl = normalizeEnrollmentServerUrl(serverUrl);
  const safeToken = normalizeEnrollmentToken(enrollmentToken);
  if (!safeServerUrl || (enrollmentToken && !safeToken)) return '';
...
function buildEnrollmentLink(serverUrl: string, enrollmentToken: string) {
  const safeServerUrl = normalizeEnrollmentServerUrl(serverUrl);
  const safeToken = normalizeEnrollmentToken(enrollmentToken);
  if (!safeServerUrl || !safeToken) return '';
...
function normalizeEnrollmentServerUrl(serverUrl) {
  if (typeof serverUrl !== 'string' || /[\s'";&|<>`$\\]/.test(serverUrl)) return '';
  try {
...
// ── Mobile input bar ───────────────────────────────────────────────────────
document.querySelectorAll('.kbd-btn[data-key]').forEach((btn: any) => {
  btn.addEventListener('click', () => {
    if (!activeSessionId) return;
...
mobileTextInput.addEventListener('input', () => {
  if (!activeSessionId || !mobileTextInput.value) return;
...
function loadSnippets() {
  try { return JSON.parse(localStorage.getItem('at_snippets') || '[]'); } catch { return []; }
}
...
function sendComposedCommand(run) {
  ...
  if (!sendTerminalInput(activeSessionId, run ? `${command}\r` : command)) return;
  input.value = '';
...
function toggleSpeechRecognition() {
  ...
  recognition.onresult = (event) => {
    if (speechRecognition !== recognition || $('#command-composer-panel').classList.contains('hidden')) return;
    let transcript = '';
...
function setComposerListening(isListening) {
  ...
  if (status) status.textContent = isListening ? 'Listening...' : 'Review dictated text before running it.';
}
...
function showPairingScreenFromHash() {
  if (!window.location.hash.startsWith('#enroll-agent?')) return false;
  ...
  if (!serverUrl || !enrollmentToken) return false;
...
$('#session-location-up').addEventListener('click', () => {
  if (locationState.parentPath) browseLocation(locationState.parentPath);
});
...
$('#session-context-refresh').addEventListener('click', () => {
  if (activeSessionId) loadSessionContext(activeSessionId);
});
...
$('#session-context-ctrlc').addEventListener('click', () => {
  if (activeSessionId) sendTerminalInput(activeSessionId, '\x03');
});
...
function applyViewportHeight() {
  ...
  requestAnimationFrame(() => {
    openTerminals.forEach((entry, id) => {
      fitTerminal(id);
      if (keyboardAppeared) entry.term.scrollToBottom();
    });
  });
}
applyViewportHeight();
...
if (showPairingScreenFromHash()) {
  ...
  apiRequest('GET', '/api/status')
    .then((data) => {
      ...
      if (data && data.publicUrl) serverPublicUrl = data.publicUrl;
      initDashboard();
    })
    .catch(() => {
      token = null;
